...
(() => {
  ...
  var _fit = function _fit2(fromState, toState, scale2, applyProps, fitChild, vars) {
  ...
  var _fromTo = function _fromTo2(fromState, toState, vars, relative) {
    ...
    vars = vars || {};
    var _vars = vars, clearProps2 = _vars.clearProps, onEnter = _vars.onEnter, onLeave = _vars.onLeave, absolute = _vars.absolute, absoluteOnLeave = _vars.absoluteOnLeave, custom = _vars.custom, delay = _vars.delay, paused = _vars.paused, repeat = _vars.repeat, repeatDelay = _vars.repeatDelay, yoyo = _vars.yoyo, toggleClass2 = _vars.toggleClass, nested = _vars.nested, _zIndex = _vars.zIndex, scale2 = _vars.scale, fade = _vars.fade, stagger = _vars.stagger, spin = _vars.spin, prune = _vars.prune, props = ("props" in vars ? vars : fromState).props, tweenVars = _copy(vars, _reserved), animation = gsap2.timeline({
      delay,
      paused,
      repeat,
      repeatDelay,
  ...
  var Flip = /* @__PURE__ */ function() {
    ...
    Flip2.fit = function fit(fromEl, toEl, vars) {
      var v = vars ? _copy(vars, _fitReserved) : {}, _ref = vars || v, absolute = _ref.absolute, scale2 = _ref.scale, getVars = _ref.getVars, props = _ref.props, runBackwards = _ref.runBackwards, onComplete = _ref.onComplete, simple = _ref.simple, fitChild = vars && vars.fitChild && _getEl(vars.fitChild), before2 = _parseElementState(toEl, props, simple, fromEl), after = _parseElementState(fromEl, 0, simple, before2), inlineProps = props ? _memoizedRemoveProps[props] : _removeProps;
      ...
      v = _fit(after, before2, scale2 || fitChild, props, fitChild, v.duration || getVars ? v : 0);
  ...
  var _getScrollNormalizer = function _getScrollNormalizer2(vars) {
    ...
    var _vars2 = vars, normalizeScrollX = _vars2.normalizeScrollX, momentum = _vars2.momentum, allowNestedScroll = _vars2.allowNestedScroll, onRelease = _vars2.onRelease, self2, maxY, target = _getTarget(vars.target) || _docEl2, smoother = gsap5.core.globals().ScrollSmoother, smootherInstance = smoother && smoother.get(), content = _fixIOSBug && (vars.content && _getTarget(vars.content) || smootherInstance && vars.content !== false && !smootherInstance.smooth() && smootherInstance.content()), scrollFuncY = _getScrollFunc(target, _vertical), scrollFuncX = _getScrollFunc(target, _horizontal), scale2 = 1, initialScale = (Observer.isTouch && _win5.visualViewport ? _win5.visualViewport.scale * _win5.visualViewport.width : _win5.outerWidth) / _win5.innerWidth, wheelRefresh = 0, resolveMomentumDuration = _isFunction5(momentum) ? function() {
    ...
    vars.onPress = function() {
      ...
      scale2 = _round7((_win5.visualViewport && _win5.visualViewport.scale || 1) / initialScale);
      tween.pause();
  ...
  var homeHero = () => {
    ...
    function heroAnimation() {
      ...
      const scale2 = pathWidth / 286;
      const pathHeight = 278 * scale2;
      const pathPosition = {
      ...
      heroTl.to(overlay, { opacity: 0, display: "none" }).from("html", { "--col-gap": "20em" }, "<").from(imageList, { opacity: 0 }, "<").from(oddSections, { translateY: "100vh" }, "<").from(evenSections, { translateY: "-100vh" }, "<").from(images, { scale: 1.5 }, "<1").to(imageList, { rotate: "-6" }, "<").fromTo(evenGroups, { x: 0 }, { x: "-100%", duration: 20, ease: "none", repeat: -1 }, "<").fromTo(oddGroups, { x: 0 }, { x: "100%", duration: 20, ease: "none", repeat: -1 }, "<").to(clipWrapper, { scale: scale2, translateX: pathPosition.x, translateY: pathPosition.y }, "< 2").to(nav, { opacity: 1, translateY: 0 }, "<").to("#clip-start", { morphSVG: "#clip-end", onComplete: heroMouse }, "<").from(
        heroSplit.lines,
        {
          duration: 1,
          opacity: 0,
